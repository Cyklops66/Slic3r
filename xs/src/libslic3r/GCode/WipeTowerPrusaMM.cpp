	...
	if (purpose == PURPOSE_EXTRUDE || purpose == PURPOSE_MOVE_TO_TOWER_AND_EXTRUDE) {
		...
		if (sideOnly) {
			...
			for (size_t i = 0; i < 4; ++ i, x_offset += m_perimeter_width)
				writer.travel (wipeTower_box.ld + xy(- x_offset,   y_offset), 2000)
					  .extrude(wipeTower_box.lu + xy(- x_offset, - y_offset), 2100);
			writer.travel(wipeTower_box.rd + xy(x_offset, y_offset), 2000);
		...
		writer.travel(wipeTower_box.ld, 2000);
	...
	const int 				 new_temperature)
{
	...
	case ABS:
		...
		writer.ram(xl + m_perimeter_width * 2, xr - m_perimeter_width,     y_step * 0.2f, 0,  1.2f * e,  2000)
			  .ram(xr - m_perimeter_width,     xl + m_perimeter_width,     y_step * 1.2f, e0, 1.6f * e,  2000)
			  .ram(xl + m_perimeter_width * 2, xr - m_perimeter_width * 2, y_step * 1.2f, e0, 1.8f * e,  2000)
			  .ram(xr - m_perimeter_width * 2, xl + m_perimeter_width * 2, y_step * 1.2f, e0, 1.8f * e,  2000);
		break;
	case PVA:
		writer.ram(xl + m_perimeter_width * 2, xr - m_perimeter_width,     y_step * 0.2f, 0,  3,     2000)
			  .ram(xr - m_perimeter_width,     xl + m_perimeter_width,     y_step * 1.5f, 0,  3,     2000)
			  .ram(xl + m_perimeter_width * 2, xr - m_perimeter_width * 2, y_step * 1.5f, 0,  3,     2000)
			  .ram(xr - m_perimeter_width,     xl + m_perimeter_width,     y_step * 1.5f, 0,  3,     2000);
		break;
	case SCAFF:
		writer.ram(xl + m_perimeter_width * 2, xr - m_perimeter_width,     y_step * 2.f,  0,  3,     2000)
			  .ram(xr - m_perimeter_width,     xl + m_perimeter_width,     y_step * 3.f,  0,  4,     2000)
			  .ram(xl + m_perimeter_width * 2, xr - m_perimeter_width * 2, y_step * 3.f,  0,  4.5,   2000);
		break;
	default:
		writer.ram(xl + m_perimeter_width * 2, xr - m_perimeter_width,     y_step * 0.2f, 0,  1.6f  * e, 2000)
			  .ram(xr - m_perimeter_width,     xl + m_perimeter_width,     y_step * 1.2f, e0, 1.65f * e, 2000)
			  .ram(xl + m_perimeter_width * 2, xr - m_perimeter_width * 2, y_step * 1.2f, e0, 1.74f * e, 2000);
	}

	// Pull the filament end into a cooling tube.
	//writer.retract(0, 5000).retract(0, 5400).retract(0, 3000).retract(0, 2000);
	...
	writer.travel(xr, writer.y() + y_step * 0.8f, 2000)
		  .suppress_preview();
	switch (current_material)
	{
	case ABS:
		writer.cool(xl, xr, 0, 0, 1600)
			  .cool(xl, xr, 0, 0, 2000)
			  .cool(xl, xr, 0, 0, 2400)
			  .cool(xl, xr, 0, 0, 2400);
		break;
	...
	const box_coordinates  &cleaning_box)
{
	...
	// so the excess material will not create a blob at a single position.
		  .suppress_preview()
		 // .load_move_x(xr, 0, 1400)
		 // .load_move_x(xl, 0, 3000)
		 // .load_move_x(xr, 0, 1600)
		 // .load_move_x(xl, 0, 1000)
		  .resume_preview();
	...
	for (int i = 0; i < pass; ++ i) {
		writer.travel (xr, writer.y() + dy, 2000);
		writer.extrude(xl, writer.y(), 		2200);
		writer.travel (xl, writer.y() + dy, 2000);
	...
	const box_coordinates  &cleaning_box)
{
	...
	float wipe_speed 	 = 1800.f;
	float wipe_speed_inc = 50.f;
	float wipe_speed_max = 2000.f;
...
WipeTower::ToolChangeResult WipeTowerPrusaMM::finish_layer(Purpose purpose)
{
	...
	if (purpose == PURPOSE_MOVE_TO_TOWER || purpose == PURPOSE_MOVE_TO_TOWER_AND_EXTRUDE) {
		if (m_idx_tool_change_in_layer == 0) {
			...
			writer.retract(m_retract * 1.5f, 3600)
				  ...
				  .z_hop(m_zhop, 2000)
				  .travel(fill_box.ld + xy(5.f + 15.f * float(rand()) / RAND_MAX, 0.f), 2000)
				  .z_hop_reset(2000)
				  // Prime the extruder.
	...
	if (purpose == PURPOSE_EXTRUDE || purpose == PURPOSE_MOVE_TO_TOWER_AND_EXTRUDE) {
		...
		if (purpose == PURPOSE_MOVE_TO_TOWER_AND_EXTRUDE)
			...
			writer.travel(fill_box.ld + xy(  m_perimeter_width, m_perimeter_width / 2), 2000)
			  	  .travel(fill_box.rd + xy(- m_perimeter_width, m_perimeter_width / 2));
		else
			writer.feedrate(2000);
